...
import type { MissionRisk, MissionSeverity } from "./mission-control-types";
...
export function assessMissionRisks(signals: BrandSignal[]): MissionRisk[] {
  const riskAreas = [
    "identity",
    "positioning",
    "trust",
  ...
  return riskAreas.map((area) => {
    ...
    return {
      area: area === "growth" ? "offer" : area,
      risk: calculateRiskSeverity(matchingSignals.length, averageStrength),
      description:
