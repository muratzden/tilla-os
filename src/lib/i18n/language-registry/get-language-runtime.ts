import type { LanguageRuntime, LanguageRuntimeInput } from "./language-runtime-types";
import type { OutputLanguage } from "@/src/lib/i18n/language";
import type { SystemLanguage } from "@/src/lib/i18n/system-languages";

import { DEFAULT_OUTPUT_LANGUAGE, resolveOutputLanguage } from "@/src/lib/i18n/language";
...
function resolveMarketplaceOutputLanguage(brandId: string): OutputLanguage | undefined {
  try {
...
export function getLanguageRuntime(input: LanguageRuntimeInput): LanguageRuntime {
  ...
  const activeOutputLanguage =
    marketplaceOutputLanguage ?? requestedOutputLanguage ?? DEFAULT_OUTPUT_LANGUAGE;
