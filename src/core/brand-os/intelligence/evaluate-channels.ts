...
export function evaluateChannels(
  ...
  if (
    input.channels.primary.length > 0 &&
  ...
  if (
    input.channels.primary.length <= 1 &&
    input.channels.experiments.length <= 2
  ) {
    score += 12;
    strengths.push("Execution complexity is contained.");
  } else {
    risks.push(
      "Execution complexity is high because too many channels or experiments are active.",
    );
    recommendations.push(
      "Reduce channel scope before increasing execution load.",
    );
  }

  if (
    input.channels.primary.length > 0 &&
    input.channels.secondary.length === 0
  ) {
    score += 10;
  ...
  if (
    input.channels.primary.length > 0 &&
    (!input.audience.primary || !input.positioning.promise)
  ) {
    risks.push("Channel selection may be premature.");
    recommendations.push(
      "Validate audience and message before expanding channels.",
    );
  } else if (input.channels.primary.length > 0) {
    score += 10;
