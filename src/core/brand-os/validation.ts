...
function validateOptionalStringArray(
  ...
  if (!Array.isArray(raw)) {
    errors.push(
      issue(field, "invalid_type", `${field} must be an array of strings.`),
    );
    return;
  }
  ...
  if (raw.length > MAX_ARRAY_LENGTH) {
    errors.push(
      issue(
        field,
        "too_long",
        `${field} must include ${MAX_ARRAY_LENGTH} items or fewer.`,
        MAX_ARRAY_LENGTH,
      ),
    );
  }
...
export function validateBrandOSInput(
  raw: unknown,
): ValidationResult<BrandSeedInput> {
...
export function validateBrandOSUpdate(
  raw: unknown,
): ValidationResult<BrandOSUpdateInput> {
  const errors: ValidationIssue[] = [];
  ...
  if (isRecord(audience)) {
    validateKnownFields(
      audience,
      ["primary", "needs", "barriers", "desiredOutcome"],
      "audience",
      errors,
    );
  ...
  if (isRecord(positioning)) {
    validateKnownFields(
      positioning,
      ["category", "promise", "differentiators", "proofPoints"],
      "positioning",
      errors,
    );
