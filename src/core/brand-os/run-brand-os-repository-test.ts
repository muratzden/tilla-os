...
import type { BrandOSFailure, BrandOSRepositoryResult, BrandOSSuccess, DecisionRecord } from "./types";
...
function expectSuccess(result: BrandOSRepositoryResult, message: string): BrandOSSuccess {
  assert(result.ok, message);
  return result as BrandOSSuccess;
}

function expectFailure(result: BrandOSRepositoryResult, message: string): BrandOSFailure {
  assert(!result.ok, message);
...
assert(success.result.state.schemaVersion === CURRENT_BRAND_OS_SCHEMA_VERSION, "Initialized state should include schemaVersion.");
...
let getSuccess = expectSuccess(getResult, "getWorkspaceState should succeed after initialize.");
assert(getSuccess.result.state.id === success.result.state.id, "getWorkspaceState should return persisted state.");
...
result = await repository.completeWorkspaceStudioStep(workspaceId, {
  ...
  output: {
    positioning: {
      ...
      differentiators: ["Workspace repository", "Schema version", "Event stream"],
...
const decision = success.result.state.decisions.find((item: DecisionRecord) => item.status === "proposed");
...
result = await repository.recordWorkspaceObservation(workspaceId, {
  observation: "A stakeholder could trace why the current positioning was chosen.",
...
assert(persistedState!.id === finalState.id, "Persisted state should match final state.");
assert(finalState.missionControl.readinessScore !== initializedReadiness, "Readiness should change after repository operations.");
assert(finalEvents.length > initializedEventCount, "Repository should append events across operations.");
assert(finalState.schemaVersion === CURRENT_BRAND_OS_SCHEMA_VERSION, "Final state should keep schemaVersion.");
assert(finalState.memory.decisions.some((item: DecisionRecord) => item.status === "accepted"), "Accepted decision should persist.");
assert(finalState.memory.observations.length > 0, "Observation should persist.");

const missingWorkspace = await repository.applyWorkspaceInput("missing_workspace", {
  audience: {
    primary: "Independent experts"
  }
});
const missingWorkspaceFailure = expectFailure(missingWorkspace, "Missing workspace update should fail cleanly.");
assert(
  missingWorkspaceFailure.errors?.some((error) => error.code === "not_found") ?? false,
...
const unsupportedResult = await repository.recalculateWorkspaceState("unsupported_workspace");
const unsupportedFailure = expectFailure(unsupportedResult, "Unsupported schema version should fail cleanly.");
assert(
  unsupportedFailure.errors?.some((error) => error.code === "unsupported_schema_version") ?? false,
