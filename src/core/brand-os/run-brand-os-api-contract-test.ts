...
assert(
  ...
  "Unknown operation should return ok false.",
);
assert(
  Array.isArray(invalidOperation.body.supportedOperations),
  "Unknown operation should include supported operations.",
);
...
assert(
  invalidInitialize.status === 400,
  "Invalid initialize payload should return 400.",
);
assert(
  invalidInitialize.body.ok === false,
  "Invalid initialize payload should return ok false.",
);
assert(
  Array.isArray(invalidInitialize.body.validationErrors),
  "Invalid initialize should include validation errors.",
);
...
response = await postJson({
  ...
  payload: {
    observation:
      "A prospect repeated the positioning back clearly after one explanation.",
...
assert(
  ...
  "Readiness should change through API flow.",
);
assert(
  memorySize(state) > initialMemorySize,
  "Memory should grow through API flow.",
);
assert(state.memory.scoreSnapshots.length > 0, "Score snapshots should exist.");
assert(
  state.memory.lifecycleTransitions.length > 0,
  "Lifecycle transitions should exist.",
);
...
assert(
  ...
  "Accepted decision should be stored.",
);
