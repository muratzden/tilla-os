...
export interface BrandOSRepository {
  ...
  applyWorkspaceInput(
    workspaceId: string,
    payload: BrandOSUpdateInput,
  ): Promise<BrandOSRepositoryResult>;
  completeWorkspaceStudioStep(
    workspaceId: string,
    payload: WorkspaceStudioStepPayload,
  ): Promise<BrandOSRepositoryResult>;
  acceptWorkspaceDecision(
    workspaceId: string,
    decisionId: string,
    selectedOptionId?: string,
  ): Promise<BrandOSRepositoryResult>;
  recordWorkspaceDecisionOutcome(
    workspaceId: string,
    payload: WorkspaceDecisionOutcomePayload,
  ): Promise<BrandOSRepositoryResult>;
  recordWorkspaceObservation(
    workspaceId: string,
    payload: WorkspaceObservationPayload,
  ): Promise<BrandOSRepositoryResult>;
  recalculateWorkspaceState(
    workspaceId: string,
  ): Promise<BrandOSRepositoryResult>;
}
...
async function loadValidState(
  ...
  if (!state) {
    return fail("Workspace state not found.", [
      {
        path: "workspaceId",
        code: "not_found",
...
export function createBrandOSRepository(
  adapter: BrandOSStorageAdapter,
): BrandOSRepository {
  return {
    async getWorkspaceState(
      ...
      return persistResult(
        adapter,
        workspaceId,
        recalculateBrandOS(loaded.state),
      );
    },
    ...
    async applyWorkspaceInput(
      ...
      return persistResult(
        adapter,
        workspaceId,
        applyBrandInput(loaded.state, payload),
      );
    },

    async completeWorkspaceStudioStep(
      workspaceId: string,
      payload: WorkspaceStudioStepPayload,
    ): Promise<BrandOSRepositoryResult> {
      const loaded = await loadValidState(adapter, workspaceId);
      ...
      return persistResult(
        adapter,
        workspaceId,
        completeStudioStep(loaded.state, payload.studioId, payload.output),
      );
    },
    ...
    async acceptWorkspaceDecision(
      ...
      return persistResult(
        adapter,
        workspaceId,
        acceptDecision(loaded.state, decisionId, selectedOptionId),
      );
    },
    ...
    async recordWorkspaceDecisionOutcome(
      ...
      return persistResult(
        adapter,
        workspaceId,
        recordDecisionOutcome(loaded.state, payload),
      );
    },

    async recordWorkspaceObservation(
      workspaceId: string,
      payload: WorkspaceObservationPayload,
    ): Promise<BrandOSRepositoryResult> {
      const loaded = await loadValidState(adapter, workspaceId);
    ...
    async recalculateWorkspaceState(
      workspaceId: string,
    ): Promise<BrandOSRepositoryResult> {
      const loaded = await loadValidState(adapter, workspaceId);
