...
const scenarios: Scenario[] = [
  {
    name: "SaaS startup",
    seed: {
      idea: "AI operations assistant for consulting teams",
      brandType: "saas",
    },
    input: {
      audience: {
      ...
      offer: {
        ...
        outcomes: ["Clear weekly priorities"],
      },
    ...
    observation:
      "Teams said the weekly rhythm sounded easier than their current process.",
  },
  {
    name: "Coffee brand",
    ...
    observation:
      "People liked the idea but could not explain what makes it different.",
  },
  {
    name: "Dental clinic",
    seed: {
...
function runScenario(scenario: Scenario) {
  ...
  const firstDecision = result.state.decisions.find(
    (decision) => decision.status === "proposed",
  );
  assert(
    !!firstDecision,
    `${scenario.name} should start with proposed decisions.`,
  );
  ...
  const decisionToAccept = result.state.decisions.find(
    (decision) =>
      decision.status === "proposed" && decision.source === "intelligence",
  );
  assert(
    !!decisionToAccept,
    `${scenario.name} should have an intelligence decision to accept.`,
  );
  ...
  assert(
    ...
    `${scenario.name} should not create duplicate active proposed decisions.`,
  );

  result = recalculateBrandOS(result.state);
  assert(
    result.state.decisions.length === onceRefreshedCount,
    `${scenario.name} repeated refresh should not create duplicates.`,
  );

  const acceptedDecision = result.state.decisions.find(
    (decision) => decision.id === acceptedId,
  );
  assert(
    acceptedDecision?.status === "accepted",
    `${scenario.name} accepted decision should remain accepted.`,
  );
  assert(
    result.state.memory.decisions.length === result.state.decisions.length,
    `${scenario.name} memory decisions should stay in sync.`,
  );
  assert(
    result.state.memory.decisions.some(
      (decision) => decision.status === "accepted",
    ),
    `${scenario.name} memory should include accepted decision.`,
  );
  ...
  return {
    ...
    recommendations: result.state.decisions
      ...
      .map(
        (decision) =>
          `${decision.area}:${decision.status}:${decision.recommendation}`,
      ),
    superseded: result.state.decisions.filter(
...
const recommendationSets = new Set(
  results.map((result) => result.recommendations.join("|")),
);
const totalSuperseded = results.reduce(
  (sum, result) => sum + result.superseded,
  0,
);
...
assert(
  results.every(
    (result) => result.afterInputCount > result.initialDecisionCount,
  ),
  "New decisions should be created after input changes.",
);
assert(
  results.some((result) => result.finalDecisionCount > result.afterStudioCount),
  "Decision count should grow after later state changes.",
);
assert(
  totalSuperseded > 0,
  "At least one proposed decision should be superseded when bottlenecks change.",
);
