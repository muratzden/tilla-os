...
import type { BrandOperatingState, BrandScore, ScoreDimension, ScoreDimensionResult, ScoreSnapshot } from "./types";
...
function dimensionFromEvaluations(evaluations: StrategicEvaluation[]): ScoreDimensionResult {
  const averageScore = evaluations.reduce((sum, evaluation) => sum + evaluation.score, 0) / evaluations.length;
  const riskPenalty = evaluations.reduce((sum, evaluation) => sum + evaluation.risks.length * 3 + evaluation.weaknesses.length * 2, 0);

  return {
    score: clampScore(averageScore - riskPenalty),
    reasons: mergeUnique(evaluations.flatMap((evaluation) => evaluation.strengths)),
    missingInputs: mergeUnique(evaluations.flatMap((evaluation) => evaluation.missingEvidence)),
    evidence: mergeUnique(evaluations.flatMap((evaluation) => evaluation.strengths)),
...
function fallbackDimension(score: number, reasons: string[], missingInputs: string[], penalties: string[] = []): ScoreDimensionResult {
  return {
...
function calculateFallbackBrandScore(state: Omit<BrandOperatingState, "score" | "missionControl" | "lifecycleStage">): BrandScore {
  return {
    ...
    differentiation: fallbackDimension(
      ...
      state.positioning.differentiators.length > 0 ? ["Differentiators are present."] : [],
    ...
    authority: fallbackDimension(
      ...
      state.authority.themes.length > 0 ? ["Authority themes are present."] : [],
      state.authority.themes.length > 0 ? [] : ["authority themes"]
    ),
    consistency: fallbackDimension(
      state.memory.decisions.some((decision) => decision.status === "accepted") ? 35 : 5,
      state.memory.decisions.some((decision) => decision.status === "accepted") ? ["Accepted decision is present."] : [],
      ["accepted decisions"]
    ),
    growthReadiness: fallbackDimension(
      state.growth.objectives.length > 0 ? 35 : 5,
      state.growth.objectives.length > 0 ? ["Growth objective is present."] : [],
...
function applyDecisionOutcomeSignals(
  score: BrandScore,
  state: Omit<BrandOperatingState, "score" | "missionControl" | "lifecycleStage">
): BrandScore {
  return state.memory.decisionOutcomes.reduce((nextScore, outcome) => {
    ...
    if (outcome.status === "validated" && impact.direction === "increase") {
      ...
      return {
        ...
        [impact.dimension]: {
          ...
          reasons: mergeUnique([...current.reasons, `Validated decision outcome: ${impact.rationale}`]),
          evidence: mergeUnique([...current.evidence, ...outcome.validationEvidence, impact.rationale])
        }
      };
    }
    ...
    if (outcome.status === "failed") {
      ...
      return {
        ...
        [impact.dimension]: {
          ...
          penalties: mergeUnique([...current.penalties, `Failed decision outcome: ${impact.rationale}`])
        }
      };
    }
    ...
    if (outcome.status === "in_progress") {
      return {
        ...
        [impact.dimension]: {
          ...current,
          evidence: mergeUnique([...current.evidence, `Decision outcome in progress: ${impact.rationale}`])
        }
      };
    }
...
export function calculateBrandScore(state: Omit<BrandOperatingState, "score" | "missionControl" | "lifecycleStage">): BrandScore {
  let intelligence: ReturnType<typeof evaluateBrandIntelligence>;

  try {
    intelligence = evaluateBrandIntelligence(state);
  } catch {
    return applyDecisionOutcomeSignals(calculateFallbackBrandScore(state), state);
  }

  return applyDecisionOutcomeSignals({
    clarity: dimensionFromEvaluations(intelligence.dimensionMap.clarity),
    audienceFit: dimensionFromEvaluations(intelligence.dimensionMap.audienceFit),
    differentiation: dimensionFromEvaluations(intelligence.dimensionMap.differentiation),
    ...
    consistency: dimensionFromEvaluations(intelligence.dimensionMap.consistency),
    growthReadiness: dimensionFromEvaluations(intelligence.dimensionMap.growthReadiness)
  }, state);
}

export function calculateReadinessScore(score: BrandScore): number {
  const total = DIMENSIONS.reduce((sum, dimension) => sum + score[dimension].score, 0);
...
export function createScoreSnapshot(score: BrandScore, now = new Date().toISOString()): ScoreSnapshot {
  return {
