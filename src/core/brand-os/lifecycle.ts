...
export function getLifecycleRequirements(
  ...
  const requirements: Record<BrandLifecycleStage, LifecycleRequirement[]> = {
    idea: [
      {
        field: "brand.idea",
        met: hasText(state.brand.idea),
        description: "Brand idea exists.",
      },
      {
        field: "brand.brandType",
        met: hasText(state.brand.brandType),
        description: "Brand type exists.",
      },
    ],
    foundation: [
      {
        field: "audience.primary",
        met: hasText(state.audience.primary),
        description: "Primary audience is defined.",
      },
      {
        field: "audience.desiredOutcome",
        met: hasText(state.audience.desiredOutcome),
        description: "Audience outcome is defined.",
      },
      {
        field: "offer.core",
        met: hasText(state.offer.core),
        description: "Core offer is defined.",
      },
    ],
    positioning: [
      {
        field: "positioning.category",
        met: hasText(state.positioning.category),
        description: "Category is defined.",
      },
      {
        field: "positioning.promise",
        met: hasText(state.positioning.promise),
        description: "Promise is defined.",
      },
      {
        field: "positioning.differentiators",
        met: state.positioning.differentiators.length > 0,
        description: "Differentiators exist.",
      },
    ],
    trust_building: [
      {
        field: "trust.signals",
        met: state.trust.signals.length > 0,
        description: "Trust signals exist.",
      },
      {
        field: "positioning.proofPoints",
        met: state.positioning.proofPoints.length > 0,
        description: "Proof points exist.",
      },
    ],
    authority_building: [
      {
        field: "authority.themes",
        met: state.authority.themes.length > 0,
        description: "Authority themes exist.",
      },
      {
        field: "authority.evidence",
        met: state.authority.evidence.length > 0,
        description: "Authority evidence exists.",
      },
    ],
    growth: [
      {
        field: "channels.primary",
        met: state.channels.primary.length > 0,
        description: "Primary channel exists.",
      },
      {
        field: "growth.objectives",
        met: state.growth.objectives.length > 0,
        description: "Growth objective exists.",
      },
      {
        field: "growth.loops",
        met: state.growth.loops.length > 0,
        description: "Growth loop exists.",
      },
    ],
    optimization: [
      {
      ...
      },
      {
        ...
        met: state.memory.decisions.some(
...
function stageIsMet(
  ...
  return getLifecycleRequirements(state, stage).every(
...
export function getLifecycleFocus(stage: BrandLifecycleStage): string {
  const focusByStage: Record<BrandLifecycleStage, string> = {
    ...
    trust_building:
      "Build credibility through proof, consistency, and reduced risk.",
    authority_building:
      "Create a recognizable point of view and evidence base.",
