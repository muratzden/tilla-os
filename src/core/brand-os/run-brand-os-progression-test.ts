...
result = recordObservation(
  result.state,
  "Two independent experts confirmed the positioning language is easier to explain.",
  "trust",
...
assert(
  afterAudienceReadiness > initialReadiness,
  "Audience input should improve readiness.",
);
assert(
  finalReadiness > initialReadiness,
  "Progression should improve readiness.",
);
assert(
  finalMemorySize > initialMemorySize,
  "Memory should grow during progression.",
);
assert(
  result.state.memory.observations.length > 0,
...
assert(
  result.state.missionControl.actionPlan.length > 0,
  "Mission Control should provide an action plan.",
);
assert(
  result.state.missionControl.readinessScore !== initialReadiness,
  "Mission Control should change after progression.",
);

const serializedState = JSON.stringify(result.state);
for (const term of forbiddenTerms) {
  assert(
    !serializedState.toLocaleLowerCase().includes(term.toLocaleLowerCase()),
