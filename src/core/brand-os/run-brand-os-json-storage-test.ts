...
const testFolder = join(process.cwd(), "work", "brand-os-json-storage-test", String(Date.now()));
...
let success = expectSuccess(result, "Repository should initialize state with JSON adapter.");
...
assert(reloadedState?.id === initialState.id, "State should persist after adapter recreation.");
...
assert(reloadedAgain?.brand.idea === initialState.brand.idea, "Returned state should be cloned and safe to mutate locally.");

const initialEvents = await adapter.getEvents(workspaceId);
assert(initialEvents.length === success.result.events.length, "Initial events should persist.");
...
success = expectSuccess(result, "Repository should update state with JSON adapter.");
const updatedReadiness = success.result.state.missionControl.readinessScore;
assert(updatedReadiness > initialReadiness, "Updated state should improve readiness.");

const eventsAfterUpdate = await adapter.getEvents(workspaceId);
assert(eventsAfterUpdate.length > initialEvents.length, "Events should append and persist.");

adapter = createJsonBrandOSStorageAdapter({ filePath });
const updatedReloadedState = await adapter.getState(workspaceId);
assert(!!updatedReloadedState, "Updated state should reload from JSON storage.");
assert(updatedReloadedState?.missionControl.readinessScore === updatedReadiness, "Updated score should persist after reload.");
assert(updatedReloadedState!.memory.scoreSnapshots.length > initialState.memory.scoreSnapshots.length, "Score snapshots should persist.");

const serializedFile = await readFile(filePath, "utf8");
for (const term of forbiddenTerms) {
  assert(!serializedFile.toLocaleLowerCase().includes(term.toLocaleLowerCase()), `Forbidden term found: ${term}`);
}
...
try {
  await createJsonBrandOSStorageAdapter({ filePath: corruptPath }).getState(workspaceId);
} catch (error) {
  corruptError = error;
}

assert(corruptError instanceof BrandOSJsonStorageError, "Corrupted JSON should fail with BrandOSJsonStorageError.");
assert(
  corruptError instanceof BrandOSJsonStorageError && corruptError.code === "malformed_json",
  ...
  JSON.stringify(
    {
      ...
      corruptedJsonError: corruptError instanceof BrandOSJsonStorageError ? corruptError.code : "missing"
    },
    null,
    2
  )
