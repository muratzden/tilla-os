...
const scenarios: Scenario[] = [
  {
    ...
    input: {
      audience: {
        ...
        needs: [
          "Reduce admin drag",
          "See operational priorities",
          "Coordinate recurring client work",
        ],
        barriers: [
          "Teams distrust generic automation",
          "Existing tools feel heavy",
        ],
    ...
    studioOutput: {
      positioning: {
        ...
        promise:
          "Help consulting teams recover billable time without heavier process",
        differentiators: [
          "Service-team workflow memory",
    ...
    observation:
      "Three teams described the weekly rhythm as easier than their current process.",
  },
  {
    name: "Coffee brand",
    seed: {
    ...
    observation:
      "A few people liked the morning idea but could not explain what makes it different.",
  },
  {
    name: "Dental clinic",
    seed: {
    ...
    observation:
      "A prospective patient said the visit-plan language made booking feel less intimidating.",
  },
  {
    name: "Boutique hotel",
    seed: {
    ...
    input: {
      audience: {
        ...
        barriers: [
          "Hotels look interchangeable",
          "Neighborhood fit is unclear",
        ],
  ...
  },
  {
    ...
    studioOutput: {
      positioning: {
        ...
        differentiators: [
          "Decision framing method",
          "Calm operating cadence",
          "Founder-specific judgment support",
        ],
...
function runScenario(scenario: Scenario) {
  ...
  const decision = result.state.decisions.find(
...
const progressionSignatures = new Set(
  results.map((result) =>
    result.progression.map((step) => step.readinessScore).join(">"),
  ),
);
const bottlenecks = new Set(
  results.map((result) => result.state.missionControl.bottleneck),
);
const recommendedStudios = new Set(
  results.map((result) => result.state.missionControl.recommendedStudio),
);

assert(
  progressionSignatures.size > 1,
  "Readiness score progression should differ across brands.",
);
assert(
  bottlenecks.size > 1,
  "Bottlenecks should not be identical for all brands.",
);
assert(
  recommendedStudios.size > 1,
  "Recommended studios should not be identical for all brands.",
);

for (const result of results) {
  const earlyStages = result.progression.filter(
    (step) => step.step !== "recalculate",
  );
  assert(
  ...
  assert(
    !!result.intelligence,
    `${result.name} should have an intelligence report.`,
  );
  ...
  for (const dimension of [
    ...
    assert(
      ...
      `${result.name} ${dimension} should have a score.`,
    );
    assert(
      Array.isArray(evaluation.strengths),
      `${result.name} ${dimension} should have strengths.`,
    );
    assert(
      Array.isArray(evaluation.weaknesses),
      `${result.name} ${dimension} should have weaknesses.`,
    );
    assert(
      Array.isArray(evaluation.risks),
      `${result.name} ${dimension} should have risks.`,
    );
    assert(
      Array.isArray(evaluation.missingEvidence),
      `${result.name} ${dimension} should have missingEvidence.`,
    );
    assert(
      Array.isArray(evaluation.recommendations),
      `${result.name} ${dimension} should have recommendations.`,
    );
  }
  ...
  for (const term of forbiddenTerms) {
    assert(
      !serialized.toLocaleLowerCase().includes(term.toLocaleLowerCase()),
...
console.log(
  JSON.stringify(
    results.map((result) => ({
      ...
      acceptedDecision: result.state.decisions.find(
