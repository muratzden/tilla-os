...
import type { BrandOperatingState, BrandOSUpdateInput, DecisionOption, DecisionRecord, StudioId } from "./types";
...
const scenarios: Scenario[] = [
  {
    name: "SaaS startup",
    seed: { idea: "AI bookkeeping assistant for freelancers", brandType: "saas" },
    input: {
      audience: {
        primary: "Freelancers who avoid bookkeeping until deadlines create stress",
      ...
      offer: { core: "Bookkeeping clarity assistant", outcomes: ["Weekly finance priorities"] },
      channels: { primary: ["Website"], experiments: ["Advisor referral briefing"] }
    },
    studioId: "positioning",
    studioOutput: {
      positioning: {
        category: "AI bookkeeping clarity assistant",
        promise: "Help freelancers know what to fix before bookkeeping becomes stressful",
        differentiators: ["Weekly issue detection", "Plain-language finance tasks"],
        proofPoints: ["Prototype review notes"]
      },
      authority: { themes: ["Financial clarity"], evidence: ["Freelancer interview synthesis"] },
      growth: { objectives: ["Increase qualified pilot requests"], loops: ["Publish finance clarity teardown"] }
    },
    observation: "Freelancers said the weekly issue view made finances feel less intimidating."
  },
  {
    name: "Coffee brand",
    ...
    input: {
      ...
      offer: { core: "Morning coffee ritual", outcomes: ["More consistent mornings"] },
    ...
    observation: "People liked the calmer morning idea but asked what proves the taste promise."
  },
  {
    name: "Dental clinic",
    ...
    input: {
      audience: {
        primary: "Adults delaying preventive dental visits because they expect judgment",
      ...
      offer: { core: "Anxiety-aware preventive care plan", outcomes: ["Lower booking anxiety"] },
  ...
  },
  {
    ...
    input: {
      ...
      channels: { primary: ["Website"], secondary: ["Partner guide"], experiments: ["Local guide partnership"] }
    },
    studioId: "growth",
    studioOutput: {
      positioning: {
...
function assertDecisionOptions(decision: DecisionRecord, scenarioName: string): DecisionOption[] {
  assert(decision.source === "intelligence", `${scenarioName} should inspect intelligence decisions only.`);
  assert(Array.isArray(decision.decisionOptions), `${scenarioName} decision should include decisionOptions.`);
  assert(
    decision.decisionOptions!.length >= 2 && decision.decisionOptions!.length <= 3,
    `${scenarioName} decision should include 2-3 strategic options.`
  );

  const strategies = new Set(decision.decisionOptions!.map((option) => option.strategy));
  assert(strategies.size === decision.decisionOptions!.length, `${scenarioName} options should differ by strategy.`);

  for (const option of decision.decisionOptions!) {
    assert(option.id.length > 0, `${scenarioName} option should have id.`);
    assert(option.label.length > 0, `${scenarioName} option should have label.`);
    assert(option.strategy.length > 0, `${scenarioName} option should have strategy.`);
    assert(option.tradeoffs.length > 0, `${scenarioName} option should include tradeoffs.`);
    assert(option.risks.length > 0, `${scenarioName} option should include risks.`);
    assert(option.expectedImpact.dimension.length > 0, `${scenarioName} option should include impact dimension.`);
    assert(["increase", "decrease", "neutral"].includes(option.expectedImpact.direction), `${scenarioName} impact direction is invalid.`);
    assert(["low", "medium", "high"].includes(option.expectedImpact.magnitude), `${scenarioName} impact magnitude is invalid.`);
    assert(option.expectedImpact.rationale.length > 0, `${scenarioName} option should explain expected impact.`);
  }

  return decision.decisionOptions!;
}

function assertNoDuplicateActiveDecisions(state: BrandOperatingState, scenarioName: string): void {
  ...
  assert(keys.length === new Set(keys).size, `${scenarioName} should not have duplicate active decisions.`);

  const optionSets = state.decisions
    .filter((decision) => decision.status === "proposed" && decision.source === "intelligence")
    .map((decision) => (decision.decisionOptions ?? []).map((option) => option.id).join("|"));
  assert(optionSets.length === new Set(optionSets).size, `${scenarioName} should not duplicate active option sets.`);
}

function assertNoForbiddenTerms(state: BrandOperatingState, scenarioName: string): void {
  ...
  assert(!found, `${scenarioName} should not include forbidden legacy or domain term: ${found}`);
}
...
function runScenario(scenario: Scenario) {
  ...
  const proposedIntelligenceDecisions = result.state.decisions.filter(
    (decision) => decision.status === "proposed" && decision.source === "intelligence"
  );
  assert(proposedIntelligenceDecisions.length > 0, `${scenario.name} should have intelligence decisions.`);
  ...
  const selectedOption = assertDecisionOptions(decisionToAccept, scenario.name)[0]!;
  result = acceptDecision(result.state, decisionToAccept.id, selectedOption.id);

  const acceptedDecision = result.state.decisions.find((decision) => decision.id === decisionToAccept.id);
  assert(!!acceptedDecision, `${scenario.name} selected decision should exist.`);
  assert(acceptedDecision.status === "accepted", `${scenario.name} selected decision should be accepted.`);
  assert(acceptedDecision.selectedOptionId === selectedOption.id, `${scenario.name} selectedOptionId should be stored.`);
  assert(acceptedDecision.selectedOption?.id === selectedOption.id, `${scenario.name} selected option should be stored.`);
  assert(
    acceptedDecision.impact.some((item) => item.includes(selectedOption.label) || item.includes(selectedOption.expectedImpact.rationale)),
    `${scenario.name} accepted decision impact should reflect selected option.`
  );
  assert(
    result.state.memory.decisions.some(
      (decision) => decision.id === acceptedDecision.id && decision.selectedOptionId === selectedOption.id
    ),
    `${scenario.name} memory decisions should store selected option.`
  );
  assert(
    result.state.memory.entries.some((entry) => entry.summary.includes(`Selected option: ${selectedOption.label}`)),
    `${scenario.name} memory entries should mention selected option.`
  );
  assert(
    result.state.memory.events.some((event) => event.type === "decision_accepted" && event.payload?.selectedOptionId === selectedOption.id),
    `${scenario.name} memory events should record selected option.`
  );

  result = recordObservation(result.state, scenario.observation, "trust", ["Decision option test"]);
  result = recalculateBrandOS(result.state);

  const refreshedIntelligenceDecisions = result.state.decisions.filter((decision) => decision.source === "intelligence");
  assert(refreshedIntelligenceDecisions.length >= proposedIntelligenceDecisions.length, `${scenario.name} should preserve decision history.`);
...
const recommendationSets = new Set(results.map((result) => result.activeRecommendations.join("|")));
