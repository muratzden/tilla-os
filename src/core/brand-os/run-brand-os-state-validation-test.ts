...
assert(
  validStateResult.valid,
  `Initialized state should be valid: ${JSON.stringify(validStateResult.errors)}`,
);
...
assert(
  corruptedStateResult.errors.some((error) => error.path === "brand.idea"),
  "Corrupted state should report brand.idea.",
);

const missingMissionControl = { ...state } as Partial<BrandOperatingState>;
delete missingMissionControl.missionControl;
const missingMissionControlResult = validateBrandOperatingState(
  missingMissionControl,
);
assert(
  !missingMissionControlResult.valid,
  "Missing missionControl should fail validation.",
);
assert(
  missingMissionControlResult.errors.some(
    (error) => error.path === "missionControl",
  ),
...
const invalidLifecycleResult = validateBrandOperatingState(
  invalidLifecycleState,
);
assert(
  !invalidLifecycleResult.valid,
  "Invalid lifecycleStage should fail validation.",
);
assert(
  invalidLifecycleResult.errors.some(
    (error) => error.path === "lifecycleStage",
  ),
...
assert(
  !invalidChannelsResult.valid,
  "Invalid channels should fail validation.",
);
assert(
  invalidChannelsResult.errors.some(
    (error) => error.path === "channels.primary",
  ),
...
assert(
  savedState?.id === state.id,
  "Persistence adapter should save and return state.",
);

await adapter.appendEvents("workspace_test", initialized.events);
const savedEvents = await adapter.getEvents("workspace_test");
assert(
  savedEvents.length === initialized.events.length,
  "Persistence adapter should append and return events.",
);
...
assert(
  missingStateResponse.status === 400,
  "API missing state should return 400.",
);
assert(
  missingStateResponse.body.ok === false,
  "API missing state should return ok false.",
);
assert(
  Array.isArray(missingStateResponse.body.validationErrors),
  "API missing state should include validation errors.",
);
...
assert(
  corruptedStateResponse.status === 400,
  "API corrupted state should return 400.",
);
assert(
  corruptedStateResponse.body.ok === false,
  "API corrupted state should return ok false.",
);
assert(
  Array.isArray(corruptedStateResponse.body.validationErrors),
  "API corrupted state should include validation errors.",
);
